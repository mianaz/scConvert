...
- format:
  ...
  midnight, and `"%Y-%m-%d"` otherwise. In the first case and if
  `digits` is not `NULL`, i.e., by default when
  ...
  using `"%OS<n>"` instead of `"%S"` in the format, see also ‘Details’.
...
scConvert:::FormatTime(time)
#> [1] "2020-08-04 21:41:48 UTC"
# }
```
