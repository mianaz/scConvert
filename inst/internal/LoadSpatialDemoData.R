...
#'
#' Load various spatial datasets from SeuratData for testing spatial data
#' conversion functionality.
#'
#' @param datasets Character vector of dataset names to load. Defaults to
#'   c("stxBrain", "stxKidney", "ssHippo")
#' @param install_missing Logical; if TRUE, install missing datasets
...
#' }
#'
#' @importFrom Seurat Assays Images
...
load_spatial_demo_data <- function(datasets = c("stxBrain", "stxKidney", "ssHippo"),
  ...
  sd_ns <- asNamespace("SeuratData")
  available_data <- get("AvailableData", envir = sd_ns)()
  ...
  for (dataset in datasets) {
    if (dataset %in% names(spatial_data_info)) {
      ...
      if (!info$name %in% get("InstalledData", envir = sd_ns)()$Dataset) {
        if (install_missing) {
          if (verbose) message("Installing ", info$name, "...")
          get("InstallData", envir = sd_ns)(info$name)
        } else {
      ...
      data(list = info$name, package = paste0(info$name, ".SeuratData"), envir = e)
      loaded_data[[dataset]] <- e[[info$name]]
...
test_spatial_roundtrip <- function(seurat_obj,
  ...
  scConvert(h5seurat_file, dest = h5ad_file, overwrite = TRUE)

  # Step 3: Convert back to h5Seurat
  if (verbose) message("3. Converting back to h5Seurat...")
  scConvert(h5ad_file, dest = h5seurat_back, overwrite = TRUE)
