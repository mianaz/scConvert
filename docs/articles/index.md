# Articles

### Getting Started

- [Conversions: h5Seurat and
  AnnData](https://mianaz.github.io/scConvert/articles/convert-anndata.md):
- [Saving and Loading Data from an h5Seurat
  File](https://mianaz.github.io/scConvert/articles/h5Seurat-load.md):
- [Command-Line Interface
  (CLI)](https://mianaz.github.io/scConvert/articles/cli-usage.md):

### Format Guides

- [Conversions: Seurat and
  Loom](https://mianaz.github.io/scConvert/articles/convert-loom.md):
- [Conversions: Seurat and
  Zarr](https://mianaz.github.io/scConvert/articles/convert-zarr.md):
- [Multimodal Data with MuData
  (h5mu)](https://mianaz.github.io/scConvert/articles/multimodal-h5mu.md):
- [h5Seurat File Format
  Specification](https://mianaz.github.io/scConvert/articles/h5Seurat-spec.md):
- [Data Mapping
  Reference](https://mianaz.github.io/scConvert/articles/data-mapping-reference.md):

### Applications

- [Multimodal: CITE-seq and
  Multiome](https://mianaz.github.io/scConvert/articles/multimodal-citeseq.md):
- [scATAC-seq Data Conversion with
  Signac](https://mianaz.github.io/scConvert/articles/scatac-signac.md):
- [Spatial Transcriptomics:
  Visium](https://mianaz.github.io/scConvert/articles/spatial-visium.md):
- [Spatial Technologies: Slide-seq, Visium HD, and
  Others](https://mianaz.github.io/scConvert/articles/spatial-technologies.md):
- [Immune Repertoire (TCR/BCR) Data with
  scConvert](https://mianaz.github.io/scConvert/articles/immune-repertoire.md):
