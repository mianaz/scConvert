...
spatial data – in a single `.h5Seurat` file. This vignette provides a
quick example followed by the format specification.
...
![](h5Seurat-spec_files/figure-html/demo-1.png)
...
Seurat v5 `Assay5` objects are automatically converted to this layout,
with layers stored as separate matrices.
...
`indices`, `indptr`. This matches the `dgCMatrix` layout in R. The
`dims` attribute records (n_rows, n_cols).
...
Group-based data frames may include a `colnames` attribute for column
ordering and a `row.names` dataset.
