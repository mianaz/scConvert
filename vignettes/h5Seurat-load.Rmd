...
```{r load_data, eval = has_SeuratData, purl = FALSE}
library(SeuratData)
...
As seen, we have a dataset with multiple components to it. Saving an object is as simple as calling `scSaveH5Seurat()`; minimally, this function takes a `Seurat` object and nothing else. Optional arguments are present for specifying a filename and whether or not you want to overwrite a preexisting file.

```{r save_data, eval = has_SeuratData, purl = FALSE}
scSaveH5Seurat(pbmc, filename = "pbmc3k.h5Seurat", overwrite = TRUE)
```

This process is quick and results in a compact on-disk file.

```{r file_size, eval = has_SeuratData, purl = FALSE}
...
```{r hfile, eval = has_SeuratData, purl = FALSE}
...
```{r index, eval = has_SeuratData, purl = FALSE}
hfile$index()
```
...
```{r structure, eval = has_SeuratData, purl = FALSE}
hfile[["assays"]]
hfile[["assays/RNA"]]
hfile[["reductions"]]
...
```{r close, eval = has_SeuratData, purl = FALSE}
hfile$close_all()
```

## Loading datasets

Reading data from an h5Seurat file is as simple as calling `scLoadH5Seurat()`; by default, it loads the entire object into memory.

```{r load_full_data, eval = has_SeuratData, purl = FALSE}
pbmc2 <- scLoadH5Seurat("pbmc3k.h5Seurat")
pbmc2
```

The loaded object preserves all components — clusters, reductions, and expression data:

```{r plot_loaded, eval = has_SeuratData, purl = FALSE, fig.width = 14, fig.height = 6}
library(patchwork)
...
```{r assay_load, eval = has_SeuratData, purl = FALSE}
...
```{r slot_load, eval = has_SeuratData, purl = FALSE}
...
```{r assay_slot_load, eval = has_SeuratData, purl = FALSE}
...
```{r reduc_load, eval = has_SeuratData, purl = FALSE}
...
```{r custom_load, eval = has_SeuratData, purl = FALSE}
pbmc2 <- scLoadH5Seurat("pbmc3k.h5Seurat", assays = list("RNA" = c("data", "scale.data")), reductions = NA, graphs = FALSE, images = FALSE)
pbmc2
```

In addition, there are four secondary parameters to `scLoadH5Seurat`: `meta.data`, `commands`, `misc`, and `tools`; these all take simple `TRUE`/`FALSE` values to control the loading of cell-level metadata, command logs, miscellaneous information, or tool-specific results, respectively.
...
Partial loading of datasets is an excellent way to limit memory usage and prevent the loading of massive datasets into memory. However, there can be instances in which a partial dataset was loaded, but then needs to be expanded with additional data from the h5Seurat file. Instead of redoing the partial load, we can make use of `scAppendData()` to add additional objects from an h5Seurat file to an already-loaded `Seurat` object. To show how this works, we'll start off by loading just the "data" slot from the "RNA" assay, but not load any dimensional reduction information or nearest-neighbor graphs.

```{r load_partial, eval = has_SeuratData, purl = FALSE}
pbmc2 <- scLoadH5Seurat("pbmc3k.h5Seurat", assays = c("RNA" = "data"), reductions = FALSE, graphs = FALSE, images = FALSE)
pbmc2
```

`scAppendData` takes the h5Seurat file, the `Seurat` object generated from `scLoadH5Seurat` and uses the four main paramters from `scLoadH5Seurat` (`assays`, `reductions`, `graphs`, and `images`). These parameters are used in the same way as `scLoadH5Seurat` with one exception: `assays` can now take `FALSE` as a value. By passing `FALSE`, we prevent other assay information from being loaded; this is useful if we only want to add other bits of data to our `Seurat` object. For example, we can choose to add only _global_ dimensional reductions to the already existing `Seurat` object.

```{r append_partial, eval = has_SeuratData, purl = FALSE}
pbmc2 <- scAppendData("pbmc3k.h5Seurat", pbmc2, assays = FALSE, reductions = NA, graphs = FALSE, images = FALSE)
pbmc2
```

The only limits to the number of times `scAppendData` can be run is when the `h5Seurat` file has run out of data not present in the `Seurat` object. Otherwise, it can be run multiple times, adding new bits of data to our `Seurat` object. Here, we fill out the rest of the "RNA" assay, but load no other information

```{r append_parital2, eval = has_SeuratData, purl = FALSE}
...
```{r append_full, eval = has_SeuratData, purl = FALSE}
...
```{r spatial_data, eval = has_SeuratData, purl = FALSE}
library(SeuratData)
...
```{r save_spatial, eval = has_SeuratData, purl = FALSE}
...
```{r plot_spatial, eval = has_SeuratData, purl = FALSE, fig.width = 8, fig.height = 6}
