...
| AnnData Zarr | `.zarr` | C binary + R streaming |
...
For formats not supported by the C binary (SOMA, SpatialData, rds), use
the R wrapper script installed with the package:

```bash
Rscript -e 'scConvert::scConvert_cli("data.h5ad", "data.rds")'
Rscript -e 'scConvert::scConvert_cli("data.soma", "data.h5ad")'
```
