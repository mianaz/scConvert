...
1. **C binary** (`scconvert`): Fast streaming conversion between all HDF5 formats
   (h5ad, h5seurat, h5mu, loom) without loading data into memory.
2. **R backend** (`scConvert_cli()`): Universal any-to-any conversion supporting
   all nine formats (h5ad, h5seurat, h5mu, loom, zarr, SOMA, SpatialData, rds, SCE).

The `scConvert_cli()` function automatically selects the fastest path: C binary
for HDF5 pairs, R streaming for zarr/loom cross-format pairs, and R hub for the rest.

### Supported formats

| Format | Extension | Notes |
|--------|-----------|-------|
| AnnData HDF5 | `.h5ad` | C binary + R streaming |
| SeuratDisk HDF5 | `.h5seurat` | C binary + R streaming |
| MuData HDF5 | `.h5mu` | C binary + R streaming |
| Loom HDF5 | `.loom` | C binary + R streaming |
| AnnData Zarr | `.zarr` | R streaming |
| TileDB-SOMA | `soma://` | R (requires tiledbsoma) |
| SpatialData | `.spatialdata.zarr` | R |
| R serialized | `.rds` | R hub |
...
For formats not supported by the C binary (zarr, SOMA, SpatialData, rds), use
the R wrapper script installed with the package:

```bash
Rscript -e 'scConvert::scConvert_cli("data.h5ad", "data.zarr")'
Rscript -e 'scConvert::scConvert_cli("data.zarr", "data.loom")'
```
