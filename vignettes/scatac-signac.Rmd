...
The `SeuratToH5AD()` function writes the peak matrix, cell metadata, and embeddings directly to h5ad format. The `ChromatinAssay` peak matrix is extracted as a standard sparse matrix during conversion.
...
When loading an h5ad file that contains a peak matrix, `LoadH5AD()` returns a standard Seurat object. You can then upgrade the assay to a Signac `ChromatinAssay` if needed.
