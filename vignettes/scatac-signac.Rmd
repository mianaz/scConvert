...
We use the 10x Genomics PBMC 10K scATAC-seq dataset (Chromium Controller, ATAC v2 chemistry). The filtered peak-barcode matrix is distributed as an HDF5 file that can be read directly by `Seurat::Read10X_h5()`.
...
`Read10X_h5()` reads the filtered peak-barcode matrix. We then create a `ChromatinAssay` and wrap it in a Seurat object.
