...
1. Save the `Seurat` object as an h5Seurat file using `scSaveH5Seurat()`
...
In addition to the two-step `scConvert()` + `scLoadH5Seurat()` workflow shown above, scConvert provides `LoadH5AD()` for loading h5ad files directly into Seurat objects without creating an intermediate h5Seurat file.
...
| Round-trip editing (load, modify, re-export) | `scConvert()` + `scLoadH5Seurat()` |
...
> **Note on multi-modal formats**: For true multi-modal interoperability, consider the [MuData/h5mu format](https://muon.scverse.org/) from the scverse ecosystem. scConvert provides native `SaveH5MU()` and `LoadH5MU()` functions for reading and writing h5mu files with no external dependencies. See `vignette("multimodal-h5mu")` for details.
