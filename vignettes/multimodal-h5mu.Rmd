...
- **`SaveH5MU()`**: Export a multi-assay Seurat object to h5mu, with automatic assay-to-modality name mapping. Works with Seurat V5 Assay5.
...
`SaveH5MU()` and `SeuratToH5MU()` export all assays to a single h5mu file using scConvert's native writer (no MuDataSeurat required). Assay names are automatically mapped to standard MuData modality names:
...
`LoadH5MU()` uses scConvert's native reader — no MuDataSeurat or Python required.
...
You can override the default mapping with `assays` (for `SaveH5MU()` / `SeuratToH5MU()`) or `assay.names` (for `LoadH5MU()`):
