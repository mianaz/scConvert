...
with no Python dependency. Zarr stores produced by `writeZarr()` are directly
...
To save a Seurat object as a Zarr store, use `writeZarr()` directly or
...
`writeZarr()` writes the following components:
...
Use `readZarr()` to read a Zarr store into a Seurat object:
...
`readZarr()` maps AnnData fields to Seurat:
...
Zarr stores written by `writeZarr()` are directly readable by Python's
...
Zarr stores created by scanpy or anndata are readable by `readZarr()`.
...
`writeZarr()` uses zlib compression by default (level 4). Compression is
...
Zarr stores data in chunks. `writeZarr()` writes single-chunk arrays by
default (the entire array is one chunk). For multi-chunk zarr stores produced
by other tools (e.g., cloud-hosted CELLxGENE Census data), `readZarr()`
...
| scConvert read | `readZarr()` | `readH5AD()` |
| scConvert write | `writeZarr()` | `writeH5AD()` |
