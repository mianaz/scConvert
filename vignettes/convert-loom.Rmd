...
In addition to `readLoom()` and `writeLoom()`, scConvert provides direct
streaming converters that bypass Seurat object construction. These are faster
for format-to-format conversion because they operate directly on the HDF5 files:

```{r streaming, eval = FALSE, purl = FALSE}
...
- [Command-Line Interface](cli-usage.html) -- convert Loom files from the shell or via `scConvert_cli()`
- [Data Mapping Reference](data-mapping-reference.html) -- complete field-by-field mapping tables for all formats
