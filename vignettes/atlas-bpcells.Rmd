---
title: "Atlas-Scale Datasets with BPCells"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Atlas-Scale Datasets with BPCells}
...
When datasets grow beyond 100K cells, in-memory loading can exhaust available
RAM. scConvert integrates with [BPCells](https://github.com/bnprks/BPCells)
to keep expression matrices on disk while still enabling full Seurat
workflows. This vignette demonstrates loading a real atlas-scale dataset from
[CELLxGENE Discover](https://cellxgene.cziscience.com/) with and without
BPCells.

## Download a CELLxGENE dataset

We use the **Cross-tissue immune cell atlas** (Domínguez Conde et al.,
*Science* 2022), T & innate lymphoid cell subset — 216,611 cells × 35,469
genes across 12 tissues (1.6 GB h5ad).
...
Without BPCells, the full expression matrix is loaded into RAM as a sparse
matrix. For 217K cells × 35K genes, this requires **~8.5 GB** of RAM.

```{r standard-load, eval = FALSE}
...
![217K immune cells — standard load](figures/atlas_std_dimplot.png){width=100%}

16 distinct immune cell types are clearly resolved on the UMAP.

```{r show-tissue-plot, eval = FALSE}
DimPlot(atlas, group.by = "tissue", label = TRUE, repel = TRUE,
        label.size = 3, pt.size = 0.05) +
  NoLegend() + ggtitle("Colored by tissue of origin")
```

![Colored by tissue of origin](figures/atlas_std_tissue.png){width=100%}

Cells from 12 tissues (blood, spleen, bone marrow, thymus, lymph nodes, etc.)
intermingle on the UMAP, confirming shared immune cell identities across
tissues.
...
Measured on the 217K-cell cross-tissue immune atlas (35K genes, 1.6 GB h5ad).
...
dominates object size at ~8.3 GB) is never materialized in R's memory. Only
metadata (~25 columns × 217K rows) and the UMAP embedding (217K × 2) are
loaded.

![Side-by-side: cell type and tissue](figures/atlas_comparison.png){width=100%}

## Demo with shipped data

Verify BPCells mode works on your system with the small shipped demo.

```{r demo-bpcells, eval = TRUE}
has_bp <- requireNamespace("BPCells", quietly = TRUE)
```

```{r bpcells-demo, eval = TRUE}
...
```{r demo-plots, eval = TRUE, fig.width = 12, fig.height = 5}
if (has_bp) {
  library(patchwork)
