...
All conversions route through Seurat as a universal intermediate. Direct
HDF5 paths bypass the hub for h5ad/h5Seurat conversions.
      ...
      ─── hub path (via Seurat)
      ╎╎╎ direct HDF5 path (no intermediate)

This hub design achieves **O(n) implementation complexity** for n
formats instead of O(n^2) pairwise converters.
...
| **Variable features** | Automatic `highly_variable` column mapping. |
| **C CLI** | Standalone binary for streaming on-disk conversion. |
