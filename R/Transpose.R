...
#' @export
#'
scTranspose <- function(x, ...) {
  UseMethod(generic = 'scTranspose', object = x)
}
...
#' \code{\link[Matrix:dgCMatrix-class]{dgCMatrix}} with the data of \code{x} transposed
#'
#' @rdname scTranspose
#' @method scTranspose dgCMatrix
#' @export
#'
scTranspose.dgCMatrix <- function(x, ...) {
...
#'
#' @rdname scTranspose
#' @method scTranspose H5D
#' @export
#'
scTranspose.H5D <- function(
  x,
  dest = GetParent(x = x),
...
#' @rdname scTranspose
#' @method scTranspose H5Group
#' @export
#'
scTranspose.H5Group <- function(
  x,
  dest = GetParent(x = x),
  dname = paste0('t_', basename(path = x$get_obj_name())),
  overwrite = FALSE,
