...
# Write one categorical column into an h5ad dataframe group.
#
# Creates /<parent_grp>/<name>/ with:
#   codes       integer array, 0-based with -1 for NA, dtype sized to n_levels
#   categories  UTF-8 string array of level names
#   attrs: encoding-type="categorical", encoding-version="0.2.0", ordered=FALSE
#
# Caller is responsible for name mapping (e.g. standardize) and for
# slicing `values` with any subset index before the call.
#
# @param parent_grp hdf5r H5Group to create the child group under (e.g. /obs)
# @param name       character(1), child group name
# @param levels     character vector, factor level labels
# @param values     integer vector, 1-based factor codes (NA becomes -1)
#
# @return Invisibly, the created categorical group
# @keywords internal
#
WriteH5ADCategorical <- function(parent_grp, name, levels, values) {
  raw <- as.integer(values)
  codes <- ifelse(is.na(raw), -1L, raw - 1L)
  n_cat <- length(levels)
  has_na <- any(is.na(raw))
  codes_dtype <- if (has_na || n_cat <= 127) {
  ...
  grp <- parent_grp$create_group(name = name)
  grp$create_dataset(name = 'codes', robj = codes, dtype = codes_dtype)
  AddAnndataEncoding(grp[['codes']], encoding_type = 'array')
  grp$create_dataset(name = 'categories',
                     robj = as.character(levels),
                     dtype = StringType('utf8'))
  AddAnndataEncoding(grp[['categories']], encoding_type = 'string-array')
  AddAnndataEncoding(grp, encoding_type = 'categorical')
  grp$create_attr(attr_name = 'ordered', robj = FALSE,
                  dtype = GuessDType(x = FALSE), space = ScalarSpace())
  invisible(grp)
}

# Seurat image-slot scale-factor names vs. AnnData/Scanpy attribute names.
# The only source of truth for this 4-entry table. Previously inlined in
# three separate sites (H5ADToH5Seurat, H5SeuratToH5AD, SeuratSpatialToH5AD);
# the slot-to-attribute and attribute-to-slot inverses are both derived from
# this vector so they cannot drift.
SCALEFACTOR_NAME_SEURAT_TO_H5AD <- c(
  spot     = "spot_diameter_fullres",
  fiducial = "fiducial_diameter_fullres",
  hires    = "tissue_hires_scalef",
  lowres   = "tissue_lowres_scalef"
)
SCALEFACTOR_NAME_H5AD_TO_SEURAT <- setNames(
  names(SCALEFACTOR_NAME_SEURAT_TO_H5AD),
      ...
      if (lib_group$exists(name = 'images')) {
        ...
        if (lib_group$exists(name = 'scalefactors')) {
          ...
          for (sf_h5ad_name in names(x = sf_src)) {
            sf_h5seurat_name <- if (sf_h5ad_name %in% names(SCALEFACTOR_NAME_H5AD_TO_SEURAT)) {
              unname(SCALEFACTOR_NAME_H5AD_TO_SEURAT[sf_h5ad_name])
            } else {
              sf_h5ad_name
            }
  ...
  TransferDF <- function(src, dname, index) {
        ...
        if (inherits(x = meta_group, what = 'H5Group')) {
          for (col in names(x = meta_group)) {
            ...
            if (IsFactor(x = meta_group[[col]])) {
              WriteH5ADCategorical(
                parent_grp = dfile[['obs']],
                name       = mapped_col,
                levels     = meta_group[[col]][['levels']][],
                values     = meta_group[[col]][['values']][index]
              )
            } else {
              # Regular column
    ...
    if (inherits(x = src, what = 'H5D')) {
      ...
      for (i in src$names) {
        ...
        if (IsFactor(x = src[[i]])) {
          WriteH5ADCategorical(
            parent_grp = dfile[[dname]],
            name       = mapped_i,
            levels     = src[[i]][['levels']][],
            values     = src[[i]][['values']][index]
          )
        } else {
          # Regular dataset
          ...
          tryCatch({
            if (IsFactor(x = meta.src[[col]])) {
              WriteH5ADCategorical(
                parent_grp = dfile[['obs']],
                name       = col,
                levels     = meta.src[[col]][['levels']][],
                values     = meta.src[[col]][['values']][]
              )
            } else {
      ...
      for (img_name in images) {
        ...
        if (img_group$exists(name = 'scale.factors')) {
          ...
          for (sf_name in sf_src$names) {
            dst_name <- SCALEFACTOR_NAME_SEURAT_TO_H5AD[[sf_name]]
            if (is.null(dst_name)) {
              dst_name <- sf_name
            }
